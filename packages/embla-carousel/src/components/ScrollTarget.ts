...
export function ScrollTarget(
  ...
  // TODO: Clean up?
  const snapBySlideIndex: { [key: number]: number } = slideRegistry.reduce(
    (acc, slideGroup, index) => ({
      ...acc,
      ...slideGroup.reduce((slideAcc, slideIndex) => {
        return { ...slideAcc, [slideIndex]: index }
      }, {})
    }),
    {}
  )
  ...
  function findTargetSnap(target: number): TargetType {
    ...
    const ascDiffsToSnaps = slidesInRange
      .map((index) => {
        const snapIndex = snapBySlideIndex[index]
        return {
