...
// TODO: Add selectors from slides and container options to make it dynamic
...
export function SsrHandler(
  ...
  function createStyles(options: OptionsType): string {
  ...
  function getStyles(): string {
    if (!isSsr) return ''
    if (!optionsBase.ssr.length) return ''

    const optionBreakpoints = optionsBase.breakpoints

    return `
      ${createStyles(optionsBase)}

      ${Object.keys(optionBreakpoints).reduce((acc, key) => {
        const optionsAtMedia = mergeOptions(optionsBase, optionBreakpoints[key])

        return `
          ${acc}
          @media ${key} {
            ${createStyles(optionsAtMedia)}
          }
        `
      }, '')}
    ` //.replace(/\s+/g, '')
