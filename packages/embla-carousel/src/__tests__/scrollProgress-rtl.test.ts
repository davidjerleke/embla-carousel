...
import {
  FIXTURE_SCROLL_PROGRESS_RTL_1,
  FIXTURE_SCROLL_PROGRESS_RTL_2
} from './fixtures/scrollProgress-rtl.fixture'

describe('➡️  ScrollProgress - Horizontal RTL', () => {
  describe('Is correct for each snap point when:', () => {
    test('Slides are WITHOUT MARGINS', () => {
      const emblaApi = EmblaCarousel(
        mockTestElements(FIXTURE_SCROLL_PROGRESS_RTL_1),
        { direction: 'rtl' }
      )
    ...
    test('Slides are WITH MARGINS', () => {
      const emblaApi = EmblaCarousel(
        mockTestElements(FIXTURE_SCROLL_PROGRESS_RTL_2),
        { direction: 'rtl' }
      )
