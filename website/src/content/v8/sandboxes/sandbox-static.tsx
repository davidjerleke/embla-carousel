...
import { SANDBOX_VANILLA_FOLDERS } from '@/content/v8/sandboxes/Vanilla/sandbox-vanilla-folders'
import { SANDBOX_REACT_FOLDERS } from '@/content/v8/sandboxes/React/sandbox-react-folders'
import { createSandboxVanilla } from '@/content/v8/sandboxes/Vanilla/create-sandbox-vanilla'
import { createSandboxReact } from '@/content/v8/sandboxes/React/create-sandbox-react'
import { sandboxStaticExtractPlugins } from '@/content/v8/sandboxes/sandbox-static-extract-plugins'
import {
  sandboxStaticExtractVanillaModules,
  sandboxStaticExtractReactModules
} from '@/content/v8/sandboxes/sandbox-static-extract-modules'
import {
  SandboxLanguageType,
  SandboxModuleType,
...
async function sandboxVanilla(
  ...
  const carouselComponent: {
    ...
    `@/content/v8/sandboxes/React/SandboxFilesSrc/${path}/EmblaCarousel`
  )
  const carouselScript: SandboxModuleType = await import(
    `@/content/v8/sandboxes/Vanilla/SandboxFilesDist/${path}/EmblaCarousel.${vanillaScriptExtension}`
  )
...
async function sandboxReact(
  ...
  const carouselScript: SandboxModuleType = await import(
    `@/content/v8/sandboxes/React/SandboxFilesDist/${path}/EmblaCarousel.${reactScriptExtension}`
  )
  const moduleScripts: SandboxModuleScriptType[] =
