...
} from '@/content/v8/sandboxes/sandbox-static-find-modules'
...
export async function sandboxStaticExtractVanillaModules(
  ...
  const localModulesPromises: Promise<SandboxModuleType>[] = localModules.map(
    (module) => {
      return import(
        `@/content/v8/sandboxes/Vanilla/SandboxFilesDist/${path}/${module}.${vanillaScriptExtension}`
      )
    }
  )
  const sharedModulesPromises: Promise<SandboxModuleType>[] = sharedModules.map(
    (module) => {
      return import(
        `@/content/v8/sandboxes/Vanilla/SandboxFilesDist/${module}.${vanillaScriptExtension}`
      )
    }
  )
...
export async function sandboxStaticExtractReactModules(
  ...
  const localModulesPromises: Promise<SandboxModuleType>[] = localModules.map(
    (module) => {
      return import(
        `@/content/v8/sandboxes/React/SandboxFilesDist/${path}/${module}.${reactScriptExtension}`
      )
    }
  )
  const sharedModulesPromises: Promise<SandboxModuleType>[] = sharedModules.map(
    (module) => {
      return import(
        `@/content/v8/sandboxes/React/SandboxFilesDist/${module}.${reactScriptExtension}`
      )
    }
  )
