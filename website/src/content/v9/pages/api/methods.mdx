...
import callingMethods_module_01 from '@/content/v9/code-snippets/api/methods/calling-methods-vanilla-01.js'
import callingMethods_html_01 from '@/content/v9/code-snippets/api/methods/calling-methods-vanilla-01.html'
import callingMethods_react_01 from '@/content/v9/code-snippets/api/methods/calling-methods-react-01.jsx'
import callingMethods_vue_01 from '@/content/v9/code-snippets/api/methods/calling-methods-vue-01.vue'
import callingMethods_solid_01 from '@/content/v9/code-snippets/api/methods/calling-methods-solid-01.jsx'
import callingMethods_svelte_01 from '@/content/v9/code-snippets/api/methods/calling-methods-svelte-01.svelte'
import typeScriptMethods_module_01 from '@/content/v9/code-snippets/api/methods/typescript-methods-vanilla-01.ts'
import typeScriptMethods_react_01 from '@/content/v9/code-snippets/api/methods/typescript-methods-react-01.tsx'
import typeScriptMethods_vue_01 from '@/content/v9/code-snippets/api/methods/typescript-methods-vue-01.vue'
import typeScriptMethods_solid_01 from '@/content/v9/code-snippets/api/methods/typescript-methods-solid-01.tsx'
import typeScriptMethods_svelte_01 from '@/content/v9/code-snippets/api/methods/typescript-methods-svelte-01.svelte'
...
<PrismHighlight
  language="ts"
  hideLabel
  code={`emblaApi.goToNext(true | false)`}
/>

Scroll to the next scroll snap if possible. When [`loop`](/docs/api/options/#loop) is disabled and the carousel has reached the last scroll snap, this method won't do anything. Set the **instant** parameter to `true` when you want the scroll to happen instantly (no scroll animation).
...
<PrismHighlight
  language="ts"
  hideLabel
  code={`emblaApi.goToPrev(true | false)`}
/>

Scroll to the previous scroll snap if possible. When [`loop`](/docs/api/options/#loop) is disabled and the carousel has reached the first scroll snap, this method won't do anything. Set the **instant** parameter to `true` when you want the scroll to happen instantly (no scroll animation).
...
<PrismHighlight
  language="ts"
  hideLabel
  code={`emblaApi.goTo(0, true | false, 'forward' | 'backward')`}
/>

Scroll to a scroll snap by its unique index. Set the **instant** parameter to `true` when you want the scroll to happen instantly (no scroll animation). If [`loop`](/docs/api/options/#loop) is enabled and the **direction** parameter is omitted, Embla Carousel will choose the closest way to the target scroll snap.
...
<Admonition type="note">
  **Note:** A scroll snap isn't equivalent to a slide. A scroll snap can hold
  ...
  [`slidesToScroll`](/docs/api/options/#slidestoscroll) is set to anything more
  than `1` or `auto` and multiple slides fit inside the viewport, a scroll snap
  will hold multiple slides.
</Admonition>
...
/>
<PrismHighlight language="ts" hideLabel code={`emblaApi.canGoToNext()`} />

Check the possibility to scroll to the next scroll snap. If [`loop`](/docs/api/options/#loop) is enabled and the container holds any slides, this will always return `true`.
...
<ApiMetaData
  ...
  secondRow={['Returns', 'boolean']}
/>
<PrismHighlight language="ts" hideLabel code={`emblaApi.canGoToPrev()`} />

Check the possibility to scroll to the previous scroll snap. If [`loop`](/docs/api/options/#loop) is enabled and the container holds any slides, this will always return `true`.
...
Get slide indexes **visible** in the carousel viewport. Honors the [`inViewThreshold`](/docs/api/options/#inviewthreshold) option.
...
<ApiMetaData
  firstRow={['Parameters', '(options?: EmblaOptionsType)']}
  secondRow={['Returns', 'EmblaEngineType']}
/>
<PrismHighlight
  language="ts"
  hideLabel
  code={`emblaApi.cloneEngine({ slidesToScroll: 'auto' })`}
/>

This method creates and returns a **new Embla engine instance** that acts as a **passive copy** of the original. The cloned engine has **no event listeners attached**, such as resize or slide change handlers.
...
Exposes most of Embla's internal functionality. This is primarily intended for plugin development or other advanced use cases where deeper access to the engine is required.
...
Hard reset the carousel after it has been initialized. This method allows for changing [options](/docs/api/options) and [plugins](/docs/api/plugins) after initializing a carousel.
...
**Subscribe** to an Embla [`event`](/docs/api/events) with a **callback**. Added event listeners will persist even if [`reinit`](#reinit) is called, either until the carousel is destroyed or the event is removed with the [`off`](#off) method.
...
**Unsubscribe** from an Embla [`event`](/docs/api/events). Make sure to pass the **same callback reference** when the callback was added with the [`on`](#on) method.
